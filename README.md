...
- As it follows the best performing model is the one that compresses the size the least, which is 5.3 times. In the results we will show plots of $p$ - pressure and $B_z$ - magnetic field on x-y grid and x-z in some cases. We will show $p$ because it has the lowest relative error and $B_z$ because it has the largest from all the physical parameters trough all the models
